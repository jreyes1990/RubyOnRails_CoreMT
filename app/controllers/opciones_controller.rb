class OpcionesController < ApplicationController
  ...
  def index
    @opciones = Opcion.where(:estado => 'A')
  end
 ...
 def inactivar_opcion
  ...
  @opcion.estado = "I"
    respond_to do |format|
      if @opcion.save
        format.html { redirect_to opciones_path, notice: "Opción Inctivada" }
        format.json { render :show, status: :created, location: @opcion }
      else
        format.html { render :new, status: :unprocessable_entity }
        format.json { render json: @opcion.errors, status: :unprocessable_entity }
  ...
  private
    ...
    def opcion_params
      params.require(:opcion).permit(:nombre, :descripcion, :icono, :path, :menu_id, :controlador, :es_catalogo)
    end
end
